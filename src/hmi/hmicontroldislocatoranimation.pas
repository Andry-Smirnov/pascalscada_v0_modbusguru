...
type
  ...
  private
    ...
    FEndValue: Double;
    FTarget: TControl;
    FXLinearScale: TLinearScaleProcessor;
    FYLinearScale: TLinearScaleProcessor;
    FTag: TPLCNumber;
    FTagLoaded: TPLCNumber;
    FMinX: Boolean;
    FMaxX: Boolean;
    FMinY: Boolean;
    FMaxY: Boolean;
    ...
    FGetPositionP0: UTF8String;
    FGetPositionP1: UTF8String;
    FGoToP0: UTF8String;
...
begin
  ...
  if FMinX and (FXLinearScale.Output < FMinXValue) then
    outX := FMinXValue
  else if FMaxX and (FXLinearScale.Output > FMaxXValue) then
    outX := FMaxXValue
  else
  ...
  if FMinY and (FYLinearScale.Output < FMinYValue) then
    outY := FMinYValue
  else if FMaxY and (FYLinearScale.Output > FMaxyValue) then
    outY := FMaxyValue
  else
...
procedure THMICustomControlDislocatorAnimation.SetPositionsAndMinMax(vP0X, vP0Y, vP1X, vP1Y: Longint);
begin
  if vP0X > vP1X then
  begin
    SetMaxX(vP0X);
...
procedure THMICustomControlDislocatorAnimation.SetControl(Acontrol: TControl);
begin
  if Acontrol = FTarget then Exit;
...
procedure THMICustomControlDislocatorAnimation.Notification(AComponent: TComponent; Operation: TOperation);
begin
