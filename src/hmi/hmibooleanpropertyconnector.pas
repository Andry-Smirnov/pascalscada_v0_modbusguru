...
procedure THMIBooleanPropertyConnector.CollectionNeedsComponentState(var CurState: TComponentState);
begin
...
procedure THMIBooleanPropertyConnector.SetObjects(AValue: TObjectWithBooleanPropetiesColletion);
begin
...
procedure THMIBooleanPropertyConnector.Notification(AComponent: TComponent; Operation: TOperation);
var
  i: Integer;
begin
  inherited Notification(AComponent, Operation);
  if (Operation = opRemove) and Assigned(FObjects) and (AComponent <> Self) then
  begin
    for i := 0 to FObjects.Count - 1 do
    begin
      if TObjectWithBooleanPropetiesColletionItem(FObjects.Items[i]).TargetObject = AComponent then
      begin
...
begin
  if FObjects.Owner is TComponent then
    for i := 0 to FObjects.Count - 1 do
    begin
      if Assigned(TObjectWithBooleanPropetiesColletionItem(FObjects.Items[i]).TargetObject) then
        TObjectWithBooleanPropetiesColletionItem(FObjects.Items[i]).TargetObject.RemoveFreeNotification(TComponent(FObjects.Owner));
    end;
...
procedure TObjectWithBooleanPropetiesColletionItem.SetInvertedResult(AValue: Boolean);
begin
