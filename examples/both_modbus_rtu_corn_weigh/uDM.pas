...
begin
  ...
  msgs[1]:='Deposito superior n�o abriu';
  msgs[2]:='Deposito superior n�o fechou';
  msgs[3]:='Balanca n�o abriu';
  msgs[4]:='Balanca n�o fechou';
  msgs[5]:='Balanca n�o descarregou';
...
begin
  ...
  repeat
    if PesoProdutoW1W2.LastSyncReadStatus=ioOk then begin
      ...
      sql := 'SELECT InsereTemp(NOW(), '+FloatToSQLNumer(pesoprod)+', 0, 0, 0, 0, 1, ''Balan�a Milho'',''Pesagens'','''','' Kg'', 1, 1);';
      MySQLConnection.ExecuteDirect(sql);
    end;
    inc(tentativas)
